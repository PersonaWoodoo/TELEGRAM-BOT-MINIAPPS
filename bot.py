...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton("🎰 ИГРАТЬ", web_app={"url": MINI_APP_URL})],
          [InlineKeyboardButton("📢 Канал", url=CHANNEL_LINK),
           InlineKeyboardButton("👥 Пригласить", switch_inline_query="Бро, зацени VILLD GIFT!
🆓 Тут дают бесплатный кейс каждые 24 часа
🎁 Выбил уже пару подарков
💰 Можно пополнить TON и звёздами
👇 Попробуй!")]]
    try:
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✅ Оплата прошла! Баланс пополнен. Зайдите в мини‑приложение.")
